...
def solve(params1: QuadrupleType, params2: QuadrupleType) -> tuple[BinaryType, BinaryType]:
    ...
    # 当判别式为负时，sqrt会产生警告并返回nan，这里忽略这些警告
...
def is_valid_position(pos: tuple[BinaryType, BinaryType]) -> bool:
    """
    检查计算结果是否有效（不包含nan或inf）
    :param pos: 两组经纬度
    :return: 是否有效
    """
    pos1, pos2 = pos
    return (np.isfinite(pos1[0]) and np.isfinite(pos1[1]) and
            np.isfinite(pos2[0]) and np.isfinite(pos2[1]))
